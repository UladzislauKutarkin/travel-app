...
const WeatherWidget = (props) => {
  ...
  const fetchWeather = (city: any) => {
    getCityWeather(city).then((weather) => {
      const _weather: any = {
        city: props.capital,
        temp: (weather["data"]["main"]["temp"] - 273.15).toFixed(0),
        feels: (weather["data"]["main"]["feels_like"] - 273.15).toFixed(0),
      ...
      <div className="weather_container">
        ...
        <div className="weather_params_all">
          ...
          <div className="weather_params_right">
            <div>
              {" "}
              <img className="weather_icon" src={weather_icon} alt="icon" />
            </div>

            <div>
              {props.feelsLike}: {feels}&#176;
            </div>
            <div>
              {props.winder}: {wind} m/s
            </div>
            <div>
